  ...
  if (countRow.count === 0) {
    ...
    const insertMany = db.transaction((trains) => {
      for (const train of trains) {
        insertStmt.run(train.id, train.name, train.from_station, train.to_station, train.departure_time, train.arrival_time, train.duration, train.journey_date, train.available_seats, train.total_seats, train.price_per_seat, train.days_running);
      }
    });
